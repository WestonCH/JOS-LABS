...
mem_init(void)
{
	...
	// // use cr4 to turn on the page extension
	// cr4 = rcr4();
  	// cr4 |= CR4_PSE;
  	// lcr4(cr4);
...
page_init(void)
{
	...
	// free pages!
	size_t i;
	// the number of  IO used
	// int io_num = EXTPHYSMEM / PGSIZE;
	// int io_num = PGNUM(PADDR(boot_alloc(0)));
	int io_num = PADDR(boot_alloc(0)) / PGSIZE;
	for (i = 0; i < npages; i++) 
	{
		// situation 1)
		if (i == 0)
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (!(*dir_ent & PTE_P))
	{
		...
		{
			struct PageInfo *tem_page = page_alloc(1);
			if (tem_page == NULL)
				return NULL;
			tem_page->pp_ref += 1;
			*dir_ent = page2pa(tem_page) | PTE_U |PTE_W | PTE_P;
		}	
	}
	pte_t *page_base = KADDR(PTE_ADDR(* dir_ent));
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	while (i < size)
	{
		pte_t *ent = pgdir_walk(pgdir, (void *)va, 1);
		*ent = (pa | perm |PTE_P);
...
boot_map_region_large(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	{
		// pte_t *ent = pgdir_walk(pgdir, (void *)va, 1);
		pte_t *ent = &pgdir[PDX(va)];
		*ent = (pa | perm | PTE_P | PTE_PS);
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	pte_t *ent = pgdir_walk(pgdir, va , 1);
	if (ent == NULL)
		return -E_NO_MEM;
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	pte_t *ent = pgdir_walk(pgdir, va, 0);
	if (!(ent && (*ent & PTE_P)) )
		return NULL;

	struct PageInfo *result = pa2page(PTE_ADDR( *ent));
	if (pte_store)
	{
		*pte_store = ent;
	}
...
page_remove(pde_t *pgdir, void *va)
{
	...
	pte_t *ent = NULL;
	struct PageInfo *tem_page = page_lookup(pgdir, va, &ent);
	if (tem_page == NULL)
		return;

	page_decref(tem_page);
