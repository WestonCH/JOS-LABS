...
trap_dispatch(struct Trapframe *tf)
{
    ...
    // LAB 3: Your code here.
	uint32_t condition = tf->tf_trapno;
	if (condition == T_DEBUG || condition == T_BRKPT)
	{
		monitor(tf);
		// return;
	}
	else if (condition == T_PGFLT)
	{
		page_fault_handler(tf);
	}
	else if (condition == T_SYSCALL)
	{
		ret_code = syscall
		(
			tf->tf_regs.reg_eax,
	...
	else if (condition == IRQ_OFFSET + IRQ_SPURIOUS) {
	...
	// LAB 4: Your code here.
	else if (condition == IRQ_OFFSET + IRQ_TIMER) 
	{
    	lapic_eoi();
    	sched_yield();
		return;
