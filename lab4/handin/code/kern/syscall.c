...
sys_exofork(void)
{
	...
	int reg;
	struct Env *env;
	if ((reg = env_alloc(&env, curenv->env_id)) < 0)
		return reg;
...
sys_env_set_status(envid_t envid, int status)
{
	...
	if(status != ENV_RUNNABLE && status != ENV_NOT_RUNNABLE)
		return -E_INVAL;
	if ((reg = envid2env(envid, &env, 1)) < 0)
		return reg;
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if((reg = page_insert(env->env_pgdir, pginfo, va, perm)) < 0)
	{
		page_free(pginfo);
		return reg;
	}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	struct PageInfo *pginfo;
	pte_t *ptep;
	if ((reg = envid2env(srcenvid, &src_env, 1)) < 0)
		return reg;
	if ((reg = envid2env(dstenvid, &dst_env, 1)) < 0)
		return reg;
	...
	pginfo = page_lookup(src_env->env_pgdir, srcva, &ptep);
	if (pginfo == NULL)
		return -E_INVAL;
	if (!((*ptep) & PTE_W) && (perm & PTE_W))
		return -E_INVAL;
	if ((reg = page_insert(dst_env->env_pgdir, pginfo, dstva, perm)) < 0)
		return reg;
	return 0;
